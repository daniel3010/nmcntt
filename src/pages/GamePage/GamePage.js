...
const GamePage = props => {
    ...
    const handleExtend = (e) => {
        if (document.getElementById("20").innerHTML === "Hiện thêm") {
            document.getElementById("20").className = "aboutBottom";
        } else if (document.getElementById("20").innerHTML === "Ẩn") {
                    ...
                    <div className={styles.gamepageContent}>
                        <header>
                            <button
                                ...
                                aria-label='Back'
                            >
                                <Arrow style={{fill: hoverState[19].hovered ? "#92f" : "#cccccc"}}
                            ...
                            <div className={styles.gameInfo}>
                                    ...
                                    <div
                                        ...
                                        <AnimatedText>
                                            <div className={textExtended ? styles.open : styles.closed}>
                                                <a href={selectedGame ? selectedGame.link : templateGame.link}
                                                   target="_blank">{selectedGame ? selectedGame.name : "Không"} Website</a>
                                                <h4>Phát
                                                    hành: {selectedGame ? selectedGame.release : templateGame.release}</h4>
                                                <h4>Nền
                                                    tảng: {selectedGame ? selectedGame.platforms : templateGame.platforms}</h4>
                                                <h4>Thể loại
                                                    chính: {selectedGame ? selectedGame.genre : templateGame.genre}</h4>
                                                <h4>Nhà phát
                                                    triển: {selectedGame ? selectedGame.developers : templateGame.developers}</h4>
                                                <h4 className={styles.lastChild}>Nhà xuất
                                                    bản: {selectedGame ? selectedGame.publishers : templateGame.publishers}</h4>
                                            </div>
                                        </AnimatedText>
                                        ...
                                        <button
                                            ...
                                            aria-label="Extend"
                                        >
                                            {extended ? "Ẩn" : "Hiện thêm"}
                                            {extended ? <Up className={styles.up}
                                                            style={{fill: hoverState[20].hovered ? "#fff" : "#cccccc"}}/> :
                                ...
                                <div className={styles.addToCart}>
                                    <div className={styles.infos}>
                                        ...
                                        <button id={selectedGame ? selectedGame.id : templateGame.id}
